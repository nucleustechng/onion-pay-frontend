		...
		<div>
			<div className="w-[25rem] sm:w-[45rem] md:w-[50rem] lg:w-[60rem] xl:w-[70rem] mt-6 mx-6">
				...
				<div className="flex flex-col gap-5 mt-6">
					...
					<div>
						{isSecondStep == "create-business" && !businessData ? (
							<CreateBusinessModal
								isVisible={showModal}
            ...
            />} */}
						{isSecondStep == "individual" && (
							<IndividualBusinessModal
								isVisible={showModal}
								onClose={async () => setShowModal(false)}
								handlerFunc={(nextstep: string) => setSecondStep(nextstep)}
							/>
						...
						{businessData && (
							<IndividualBusinessModal
								isVisible={showModal}
								onClose={async () => setShowModal(false)}
								handlerFunc={(nextstep: string) => {
									if (businessData) {
										toast.error(
											"Step 1 has been completed already. Start verification"
										);
									} else {
										setSecondStep(nextstep);
									}
								}}
							/>
						)}

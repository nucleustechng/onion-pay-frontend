...
import {
	...
	useLoadBanksQuery,
	useUpdateBankAccountDetailsMutation,
...
const EditBankAccountDetails = ({ isVisible, onClose }: Props) => {
	...
	useEffect(() => {
		...
		if (updateBankSuccess && bankDetailSuccess) {
			if (updateBankData.success === true) {
				toast.success(
					"Your bank account details have been updated succesfully!"
				);
				setTimeout(() => {
					onClose();
				}, 1000);
			} else {
				toast.error(updateBankData.reason);
			}
		}
	}, [updateBankSuccess]);
				...
				<div className="w-[29rem] md:w-[33rem] h-[19rem] mt-36 mb-6 rounded-[0.63rem] bg-white">
					<div className="flex flex-col mx-6 mt-6 ">
						<div className="flex items-center justify-between">
							<h1 className="text-lg text-[#616161] font-WorkSans font-semibold leading-5">
						...
						<div className="flex flex-col gap-4 mt-6">
							<div className="relative">
								<select
									className="w-[26rem] md:w-[30rem] h-[3.313rem]  focus:caret-primary outline-none 
                        ...
                        Cancel
							...
							<button
								...
								className="w-[6.21rem] h-11 bg-[#3063E9] rounded-[0.313rem] text-base text-white font-WorkSans font-normal leading-5"
							>
								<div className="flex justify-center items-center">
									{updateBankLoading ? <Loader isWhite={true} /> : "Save"}
								</div>
							</button>
						</div>
					</div>

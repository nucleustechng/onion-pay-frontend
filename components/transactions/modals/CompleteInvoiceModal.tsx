...
const CompleteInvoiceModal = ({ isVisible, onClose, data }: Props) => {
	...
	const [createInvoice, { data: createInvoiceData, isSuccess, isLoading }] =
	...
	const handleCreateInvoice = async () => {
		try {
			...
			if (
				dataToSend.order.length > 0 &&
				Object.values(dataToSend).every((value) => value !== undefined)
			) {
				await createInvoice(dataToSend);
			} else {
				toast.error(
					"Please add at least one item to the order and fill in all invoice information"
				);
			}
		} catch (err) {
			console.log(err);
	...
	useEffect(() => {
		...
		if (isSuccess && createInvoiceData?.success) {
			...
			toast.error(createInvoiceData?.reason);
		}
	...
	return (
        ...
        0 bg-opacity-50 backdrop-blur-[0.05rem] flex justify-center items-center overflow-y-scroll"
				id="wrapper"
				onClick={handleClose}
			>
				<div
					className={`w-[22.5rem] md:w-[27rem]  lg:w-[33rem] ${
						invoiceData?.order.length > 0 ? "h-[45rem]" : "h-[39.71rem]"
					} mt-32 mb-6 rounded-[0.63rem] bg-white`}
				>
					<div className="mx-4 lg:mx-6 mt-7">
						...
						<div className="flex items-center gap-4 md:gap-5 lg:gap-[4.15rem] mt-8">
							<div className="flex justify-center items-center w-[5.32rem] h-5 border-solid border-[0.07rem] border-[#3063E9] rounded-[1.25rem]">
								<h1 className="text-[#3063E9] text-sm font-WorkSans font-normal leading-4">
									Step 2 of 2
								</h1>
							</div>
							<div>
								<h1 className="text-base lg:text-lg text-[#262626] font-WorkSans font-medium leading-5">
									Item Information
								</h1>
							</div>
						</div>
						<div
							className={`${
								invoiceData?.order.length > 0 ? "h-[29rem]" : "h-[26rem]"
							} scrollbar-hide overflow-y-scroll`}
						>
							<div
								className={`flex flex-col px-4 py-3 gap-6 mt-6 w-auto h-[22rem]  border-[0.0625rem] border-primary rounded-md`}
							>
								<div className="flex items-center justify-between ">
                      ...
                      /> */}
							...
							{invoiceData.order.map((item: any, index: any) => (
								<div
									key={index}
									className={`flex flex-col px-4 py-3 gap-6 mt-6 w-auto ${
										open[index]
											? "h-[22rem] transition-all duration-500 ease-in"
											: "h-[3.25rem] transition-all ease-out duration-500"
                      ...
                      />
						...
						<div
							className={`flex justify-between items-center ${
								invoiceData?.order.length > 0 ? "mt-6" : "mt-2"
							}`}
						>
